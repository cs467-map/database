...
--\copy (SELECT json_agg(json) FROM json) To 'voyager-index-data.json'

-- https://ethanmick.com/outputting-a-postgres-table-as-json/
\copy (SELECT array_to_json(array_agg(row_to_json(t))) FROM (SELECT * FROM json) t) TO 'voyager-index-data.json'
;
